...
class Eye{
  ...
  whiteEyes(){
    ...
    fill(255);
     ...
     let wwX=constrain(this.x,(windowWidth/4)*(this.n-2)+this.r,(windowWidth/4)*(this.n-1)-this.r);
      //let wwY=constrain(this.y,(windowWidth/4)*j,(windowWidth/4)*(j+1));
      ellipse(wwX,this.y,2*this.r,2*this.r);
   

  //   for(let j=0;j<4;j++){
  //   this.x=this.x+(pose.nose.x-this.x)*this.wV;
  //   this.y=this.y+(pose.nose.y-this.y)*this.wV;
  //   let wwX=constrain(this.x,(windowWidth/4)*j,(windowWidth/4)*(j+1));
  //   //let wwY=constrain(this.y,(windowWidth/4)*j,(windowWidth/4)*(j+1));
  //   ellipse(wwX,this.y,2*this.r,2*this.r);
  // }
  }
  ...
  this.x=pose.nose.x;
  this.y=pose.nose.y;
  let bX=constrain(this.x, (windowWidth/4)*(this.n-2)+1.4*this.r, (windowWidth/4)*(this.n-1)-1.4*this.r);
  let bY=constrain(this.y, pose.nose.y-0.4*this.r, pose.nose.y+0.4*this.r);
  // let bX=constrain(this.x, pose.nose.x-0.4*this.r, pose.nose.x+0.4*this.r);
  // let bY=constrain(this.y, pose.nose.y-0.4*this.r, pose.nose.y+0.4*this.r);
